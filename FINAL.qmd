...
ui <- fluidPage(
  titlePanel("XGBoost Diabetes Prediction"),
  ...
  sidebarLayout(
    ...
    mainPanel(
      ...
      plotOutput("probGauge"),
      plotOutput("topInputsBar"),
      plotOutput("confidenceBar"),
...
server <- function(input, output) {
  observeEvent(input$predict, {
    tryCatch({
      ...
      # Predict
      pred <- predict(xgbFit, newdata = newdata, type = "prob")
      output$result <- renderPrint({
        list(
          Inputs = newdata[, colnames(newdata)[which(newdata == 1, arr.ind = TRUE)[, "col"]]],
          Probability = round(pred, 4),
          Predicted_Class = ifelse(pred[, "Yes"] > 0.5, "Yes", "No")
        )
      })
    }, error = function(e) {
      output$result <- renderPrint({
