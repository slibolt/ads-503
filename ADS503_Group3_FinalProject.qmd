...
cat_vars <- c(
  ...
  "_IMPRACE",  # Race
  "_RFHLTH",   # General health risk status
  "_HCVU653",  # Under 65 with healthcare coverage
  "_PACAT3",   # Physical activity category
  "_RFHYPE6",  # Hypertension risk
  "_CHOLCH3",  # Cholesterol check status
  "_MICHD",    # Myocardial infarction or CHD
  "_ASTHMS1",  # Asthma status
  "_DRDXAR2",  # Arthritis diagnosis
  "_RFBMI5",   # BMI risk
...
```{r}
nzv <- nearZeroVar(df_clean[, setdiff(names(df_clean), "DIABETE4")])
...
xgb_features <- c(
  ...
  "IMPRACE.1", "IMPRACE.2", "IMPRACE.3", "IMPRACE.4", "IMPRACE.5", "IMPRACE.6",
  "RFHLTH.1", "RFHLTH.2", "RFHLTH.9",
  "HCVU653.1", "HCVU653.2", "HCVU653.9",
  "PACAT3.1", "PACAT3.2", "PACAT3.3", "PACAT3.4", "PACAT3.9",
  "RFHYPE6.1", "RFHYPE6.2", "RFHYPE6.9",
  "CHOLCH3.1", "CHOLCH3.2", "CHOLCH3.3", "CHOLCH3.9",
  "MICHD.1", "MICHD.2",
  "ASTHMS1.1", "ASTHMS1.2", "ASTHMS1.3", "ASTHMS1.9",
  "DRDXAR2.1", "DRDXAR2.2",
  "AGE80",
  "RFBMI5.1", "RFBMI5.2", "RFBMI5.9",
  "SMOKER3.1", "SMOKER3.2", "SMOKER3.3", "SMOKER3.4", "SMOKER3.9"
)
...
server <- function(input, output) {
  ...
  prediction_data <- reactive({
    ...
    # Activity
    newdata[[paste0("PACAT3.", input$activity)]] <- 1
    ...
    # Race/Ethnicity
    newdata[[paste0("IMPRACE.", input$race)]] <- 1
